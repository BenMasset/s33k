...
const setup = {
   ...
   connectClients: 'Per-client connect instructions. The hosted HTTP endpoint is <base-url>/api/mcp, here https://app.s33k.io/api/mcp. '
      + 'There are TWO ways to connect, and which one you use depends on the client. '
      + 'EASIEST, NO KEY (normal Claude and ChatGPT): these consumer apps connect over OAuth, so you add ONLY the URL and sign in, '
      + 'there is no API key to paste. '
      + 'NORMAL CLAUDE (claude.ai web or the Claude desktop app): open Settings (or Customize) > Connectors > Add custom connector, '
      + 'paste the URL https://app.s33k.io/api/mcp, click Add, then Connect, and sign in with the EMAIL YOU WERE INVITED WITH. That email '
      + 'links you to your s33k account. '
      + 'CHATGPT (Plus, Pro, Business, or Enterprise): turn on Developer Mode (Settings > Apps & Connectors > Advanced settings > Developer mode), '
      + 'then Create a connector, paste the URL https://app.s33k.io/api/mcp, choose OAuth for authentication, create it, and authorize with the '
      + 'email you were invited with. On Business/Enterprise an admin may need to enable custom connectors first. '
      + 'KEY-BASED (developer clients that let you paste a header): Claude Code, Cursor, and Codex use your s33k API key directly. '
      + 'CLAUDE CODE (CLI): run `claude mcp add --transport http s33k https://app.s33k.io/api/mcp --header "Authorization: Bearer <key>"`. '
      + 'CURSOR: add to ~/.cursor/mcp.json (or project .cursor/mcp.json): { "mcpServers": { "s33k": { "url": "https://app.s33k.io/api/mcp", '
      + '"headers": { "Authorization": "Bearer <key>" } } } }. '
      + 'CODEX (CLI): run `codex mcp add s33k --url https://app.s33k.io/api/mcp --bearer-token-env-var S33K_API_KEY`, then export S33K_API_KEY=<key> '
      + 'in the shell that launches Codex (add it to ~/.zshrc to persist). '
      + 'CLAUDE DESKTOP: the simplest path is the OAuth custom-connector flow above (same as normal Claude). If you prefer a key, '
      + 'bridge it with mcp-remote in claude_desktop_config.json '
