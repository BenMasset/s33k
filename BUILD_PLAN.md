...
- [x] Package so a friend can install: `deploy/docker-compose.yml` (s33k + Umami + Postgres in one stack) and `deploy/.env.template` (placeholders and throwaways only, with openssl regenerate instructions; zero real secrets). README rewritten with the three-pillar story, install steps, and the full 19-tool MCP table.
