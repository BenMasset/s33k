...
## The AI Visibility Funnel: the headline novel feature (21st MCP tool). DONE
The one genuinely novel synthesis nobody else ships: a marketer's standing in AI search, measured by what AI engines actually DO on the site (first-party, un-gameable), never by querying an LLM. It joins the two signals s33k already collects, AI CRAWLS (which engines are learning about you, the leading indicator) and AI REFERRALS (which engines actually recommend you, the outcome), into one funnel per engine and per page.
- [x] **`GET /api/ai-visibility?domain=...&period=...`** (Bearer-key reachable, whitelisted in `utils/allowedApiRoutes.ts`, GET-only). Follows the wired analytics-route pattern: `authorize()` then a `Domain.findOne({ domain, ...scopeWhere(account) })` ownership gate (403 if not owned), exactly like `scoreboard.ts`. Reuses the existing read paths and classifiers rather than duplicating them: crawls from `CrawlerHit` (same query/period parsing as `ai-crawlers.ts`, kept per-path), referrals from `getAnalyticsProvider().getReferralSources()` filtered to `isAI` (same path as `ai-referrals.ts`). The two halves join on one engine name via an `OWNER_TO_ENGINE` map (OpenAI->ChatGPT, Anthropic->Claude, Google->Gemini, ...).
- [x] **The synthesis.** Per page a status of `ai-visible` (crawled AND cited), `crawled-not-cited` (the prime opportunity), `cited-not-crawled` (rare), or `ai-invisible`. Per engine a status of `advocate`, `aware-not-recommending`, or `absent`. Plus a funnel summary (totalAICrawls, totalAIReferrals, crawlToReferralRate, topAdvocate engine, biggestGap engine).
- [x] **Never 500s on a sub-signal failure.** Crawler read and referral read each degrade into their own error field; the citability audit is wrapped so it can never break the route. Honest about provider limits: when the provider reports referrals only site-wide (no landing path), per-page `isCited` is not attributed and a `note` flags it, while engine-level referrals and totals stay accurate.
- [x] **Optional enrichment: the AI-citability audit** (`utils/citability-audit.ts`). When first-party data is thin (`< 10 crawls && 0 referrals`), the route fetches the top pages and deterministically scores AI-readiness (llms.txt, Markdown twin, JSON-LD, answer-shaped content), 25 points each. Reuses the SSRF-guarded, timeout-bounded `safeFetchText` from `utils/site-crawl.ts` (now exported alongside `isPublicHostname`); rejects HTML masquerading as a `.md` twin so a soft-404 cannot inflate the score. Live true-positive on getmasset.com: 100/100 (llms.txt + .md twins + JSON-LD on every page). Full demo artifact: `NIGHT_DEMO_ai_visibility.md`.
- [x] **MCP tool `ai_visibility`** (the 21st tool, rich self-explaining description). Verified over stdio: `tools/list` returns 21 tools with `ai_visibility` present; the stderr banner now reads "21 tools registered." Complements `ai_crawlers` (raw crawl detail) and `ai_referrals` (raw referral detail) by joining them into one funnel.
