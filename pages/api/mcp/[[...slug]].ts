 ...
 *   request and binds a per-request fetchImpl to THAT key. The s33k API's authorize() then does all
 *   the scoping: a scoped share key (ApiKey.scoped_domain set) is held to GET-only, to the curated
 *   per-domain allowlist, and to its one domain, EXACTLY as a direct REST call would be. There is no
 *   code path in which a connection uses anything but its own key. A request with no Bearer key is
 *   rejected 401 before any MCP work happens.
 *
