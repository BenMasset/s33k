 ...
 *
 * The route returns the s33k tracking snippet + per-platform install guides for an already-onboarded,
 * caller-owned domain. This test pins the user-facing response SHAPE: the site id is surfaced under
 * the s33k-branded `siteId` key, and the internal provider name never leaks as a response key
 * (`umamiWebsiteId` is absent). The DB column read (Domain.umami_website_id) is an internal detail and
 * is exercised through the mocked owned-domain row, not asserted as output.
 *
...
// install-instructions -> install-guides -> utils/umami imports the sequelize-typescript Domain model
// transitively; mock it away so jest never has to transform the decorator syntax (same pattern as
// install-guides.test.ts). The route never touches the model directly (it reads the mocked owned row).
...
describe('GET /api/install-instructions response shape', () => {
   it('surfaces the site id under the s33k-branded `siteId` key and never leaks `umamiWebsiteId`', async () => {
      mockResolveDomainAccess.mockResolvedValue({ domain: 'getmasset.com', umami_website_id: 'web-123' });
      ...
      expect(payload.siteId).toBe('web-123');
      expect(payload).not.toHaveProperty('umamiWebsiteId');
      // The snippet embeds the resolved site id, and no user-facing string carries the provider name.
      expect(payload.installSnippet).toContain('data-website-id="web-123"');
