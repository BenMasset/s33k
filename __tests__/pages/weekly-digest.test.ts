 ...
 *   1. Ownership gate (MULTI_TENANT = 'true'): a tenant asking about a domain it does NOT own gets
 *      a 403 and NO pillar store is queried (S33kEvent / Keyword / Goal findAll/findOne not called).
 *   2. When the tenant DOES own the domain, the event + keyword reads carry the tenant owner_id
 *      scope, and the report 200s.
 *   3. Admin is unscoped: no owner_id key on the ownership lookup or the pillar reads.
 *   4. Sections compose correctly off ONE sessionized event set: traffic (human-only by default,
 ...
 *   5. rankMovers parses each keyword's history JSON for the in-window delta (improved vs worsened),
 *      reusing the rank-movers helper. Smaller position is better, so a climb is a positive delta.
 *   6. Goal sections are OPTIONAL: with no goal, conversions + topOpportunity are null and Goal is
 ...
 *   7. A requested-but-missing goal -> 404, and no event/keyword read happens.
 *   8. Degrades on an empty event set: 200 with zero human visitors and a tracking-install note.
 *   9. Missing domain -> 400, nothing read. Non-GET -> 405.
 *
 * Models are mocked, authorize is mocked per-test, and scopeWhere runs for real so the scoped
 * where-clause is the genuine flag-gated output. No network, no DB, no real models imported.
 */
...
describe('GET /api/weekly-digest: ownership gate', () => {
   it('403s a tenant requesting a domain it does NOT own and never reads any pillar store', async () => {
      asCaller(TENANT);
      mockDomain.findOne.mockResolvedValue(null);
      const res = makeRes();

      await weeklyDigestHandler(makeReq({ domain: 'someone-elses.com' }), res);
   ...
   it('scopes the event AND keyword reads by owner_id when the tenant DOES own the domain', async () => {
      asCaller(TENANT);
      mockDomain.findOne.mockResolvedValue({ ID: 7, domain: 'a.com', owner_id: TENANT.ID });
      mockEvent.findAll.mockResolvedValue([ev({})]);
...
describe('GET /api/weekly-digest: optional goal sections', () => {
   ...
   it('adds conversions total/rate and the top opportunity when a goal is supplied', async () => {
      ...
      expect(res.statusCode).toBe(200);
      expect(mockGoal.findOne.mock.calls[0][0].where).toMatchObject({ domain: 'a.com', name: 'Demo Booked', owner_id: TENANT.ID });
      const body = res.payload as Record<string, unknown>;
