 ...
 *   1. Ownership gate (MULTI_TENANT='true'): a tenant asking about a domain it does
 *      NOT own gets 403 and NO pillar is read (events/keywords never queried).
 *   2. Owned domain: event + keyword reads carry the tenant owner_id scope; admin is
 *      unscoped (no owner_id key on any where).
 *   3. Human-only by default: bot sessions are excluded from humanVisitors and
 *      topChannel.
 *   4. No goal: headline.conversions is null, topConvertingChannel is null, SEO + AI
 *      blocks still answer, and nextAction falls back to an SEO/traffic signal.
 *   5. With a goal: conversions + conversionRatePct populate, topConvertingChannel is
 *      the highest-RATE converting channel, and nextAction is the conversion money
 *      move when an opportunity exists.
 *   6. SEO block: keywordsOnPageOne counts current page-one positions; biggestGain /
 *      biggestLoss come from keyword.history over the period.
 *   7. AI visibility: sendingVisitors + count + topEngine come from AI referral sources.
 *   8. Degrades: empty everything -> 200 with zeroed numbers and an install note;
...
describe('GET /api/executive-summary: ownership gate + scoping', () => {
   it('403s a tenant requesting a domain it does NOT own and reads no pillar', async () => {
      asCaller(TENANT);
   ...
   it('scopes the event + keyword reads by owner_id when the tenant owns the domain', async () => {
      asCaller(TENANT);
...
describe('GET /api/executive-summary: with a goal', () => {
   ...
   it('goal lookup is tenant-scoped', async () => {
      asCaller(TENANT);
      mockDomain.findOne.mockResolvedValue({ ID: 7, domain: 'a.com', owner_id: TENANT.ID });
      mockGoal.findOne.mockResolvedValue(goalRow);
      const res = makeRes();
      ...
      expect(mockGoal.findOne.mock.calls[0][0].where).toMatchObject({ domain: 'a.com', owner_id: TENANT.ID, ID: 5 });
   });
});
