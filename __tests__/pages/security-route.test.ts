 ...
 *   3. The facts object covers every promised trust pillar (no-training, tenant isolation,
...
describe('securityFacts structured source', () => {
   ...
   it('points the tenant-isolation fact at the adversarial isolation test files', () => {
      const isolation = securityFacts.facts.find((f) => f.id === 'tenant_isolation');
      expect(isolation).toBeDefined();
      const proof = isolation!.verifyIn.join(' ');
      expect(proof).toContain('utils/scope.ts');
      expect(proof).toContain('route-scope-isolation.test.ts');
      expect(proof).toContain('account-routes-isolation.test.ts');
   });

   it('points the data-ownership fact at the export + hard-delete routes and MCP tools', () => {
      ...
      expect(proof).toContain('pages/api/account-data.ts');
      expect(ownership!.answer).toContain('export_data');
      expect(ownership!.answer).toContain('delete_account_data');
   });

   it('names the three sub-processors (Railway, Umami, Serper)', () => {
      const names = securityFacts.subProcessors.map((p) => p.name);
      expect(names).toEqual(expect.arrayContaining(['Railway', 'Umami (self-hosted)', 'Serper']));
   });
});
