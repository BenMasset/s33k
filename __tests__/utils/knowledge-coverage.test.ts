 ...
 *   - crossCheckCapability (the request_feature safety net) pushes back on an existing
 *     capability and lets a genuinely-novel request through.
 *
...
describe('MCP knowledge coverage: every registered tool is documented', () => {
   ...
   it('actually found the registered tools in the MCP source (sanity guard)', () => {
      ...
      expect(registeredTools).toEqual(expect.arrayContaining(['help', 'request_feature', 'list_feature_requests']));
      expect(registeredTools).toEqual(expect.arrayContaining(['list_domains', 'add_keyword', 'ai_visibility', 'briefing']));
...
describe('request_feature safety net (crossCheckCapability)', () => {
   ...
   it('never maps a request onto the meta/self-support tools', () => {
      // A request that mentions "help" or "request a feature" must not resolve to help/request_feature.
      const match = crossCheckCapability('I want a feature to request features more easily');
      if (match.matched) {
         expect(['help', 'request_feature', 'list_feature_requests']).not.toContain(match.capability!.toolName);
      }
   });
...
describe('admin-only tools document their admin restriction in the knowledge layer', () => {
   // The route-level admin gate (GET /api/feature-request -> 403 for non-admin) is enforced and
   // tested at the handler level in feature-request-routes.test.ts. Here we pin the matching
   // documentation contract so the knowledge layer the LLM reads stays in lockstep with the gate:
   // an admin-only tool must say so, so an LLM never tells a member it can list requests or waitlist.
   const adminOnlyTools = ['list_feature_requests', 'list_waitlist'];

   it.each(adminOnlyTools)('capability "%s" is documented as admin-only', (toolName) => {
      const entry = knowledge.capabilities.find((c) => c.toolName === toolName);
      expect(entry).toBeDefined();
      const doc = `${entry!.description} ${entry!.whenToUse}`.toLowerCase();
