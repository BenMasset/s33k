 ...
 * live database. The Goal model shipped that way once. This static check asserts every model file
 * is both imported AND present in the models array.
 */
describe('database model registration', () => {
   ...
   const modelFiles = fs.readdirSync(modelDir).filter((f) => f.endsWith('.ts'));
   const arrayMatch = dbSrc.match(/const models = \[([^\]]*)\]/);
   const modelsArray = arrayMatch ? arrayMatch[1] : '';

   it('imports and registers every file in database/models/', () => {
      expect(arrayMatch).not.toBeNull();
      for (const file of modelFiles) {
         const base = file.replace('.ts', '');
         // Each model file must be imported by path.
