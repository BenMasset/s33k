...
| A12 | Google Ads OAuth | open | `/api/adwords` has a public OAuth callback without a signed state, unlike Search Console OAuth. | Add a signed state helper or document why Google Ads remains admin-only and lower risk. Prefer matching the GSC pattern. |
| A13 | Route status codes | open | Several routes still return 502 for unsupported methods or 200 for errors. | Standardize on 405 for method mismatch and 4xx/5xx based on actual failure. Do this route family by route family with tests. |
| A14 | Console logging | open | Client services and scraper utilities still use ad hoc `console.log`. | Replace noisy client logs with no-ops or typed error handling. Keep intentional server warnings. |
...
| A19 | Deployment docs | open | `DEPLOY.md` and older resume docs contain historical SQLite/Railway-volume guidance. | Add a prominent current-state note or archive the old SQLite path to avoid another deploy loop. |
