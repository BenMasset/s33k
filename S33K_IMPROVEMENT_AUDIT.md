...
| A11 | Settings storage | done | The two GLOBAL file-backed state files (`data/settings.json`, `data/failed_queue.json`) moved into Postgres. | New `setting` table (one global row, id=1) holds the encrypted settings blob via `utils/settingsStore.ts` (one-time, race-safe findOrCreate import of any existing settings.json, then file never read again). The failed-retry queue is now DERIVED from `keyword.lastUpdateError` (`failedRetryWhere` / `getFailedRetryKeywordIds` in `utils/scraper.ts`); `retryScrape`/`removeFromRetryQueue` are no-ops kept for call-site compatibility; the hourly retry is `POST /api/cron?mode=retry` (tenant-scoped + spend-braked, no new whitelist needed). `cron.js` is now a file-free env scheduler (`SCRAPE_INTERVAL`/`NOTIFICATION_INTERVAL`). All three settings readers (settings.ts, searchConsole.ts, adwords.ts/api) and clearfailed.ts are off the files. Grep confirms zero remaining file reads/writes except the one-time import constant. Tests added: `__tests__/utils/settings-store.test.ts`, `__tests__/utils/failed-retry-query.test.ts`, `__tests__/pages/cron-retry-failed.test.ts` (plus updates to settings/cron/clearfailed/searchConsole/ensure-synced suites). Gate green: `npm run lint`, `npx jest --ci` (132 suites / 1158 tests), `npm run build` (Compiled successfully), `cd mcp && npm run build`. Flag-off single-tenant unchanged. |
...
|---|---|---|---|
| `/api/settings` | cookie/admin global settings | Reads/writes the global `setting` Postgres row (A11; was `data/settings.json`) for credentials and scraper settings. | Per-tenant settings would be a separate owner-scoped table, then migrate to `authorize()`. |
| `/api/clearfailed` | cookie/admin maintenance | Clears the scraper retry queue, now DB-derived (resets `keyword.lastUpdateError`; A11, was `data/failed_queue.json`). | Make the per-keyword retry clear tenant-aware (owner-scoped) before tenant access. |
