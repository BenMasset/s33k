...
- **MCP server:** `cd mcp && npm run build`, then probe over a real stdio handshake. 41 tools + 5
  resources today. Banner reads "41 tools and 5 resources registered." Smoke harness: `npm run smoke`
  from `mcp/`.
...
- Tools are registered in `mcp/src/index.ts` (41 tools + 5 resources today).
