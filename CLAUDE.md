...
- **MCP server:** `cd mcp && npm run build`, then probe over a real stdio handshake. 58 tools + 5
  resources today. Banner reads "58 tools and 5 resources registered." Smoke harness: `npm run smoke`
  from `mcp/`.
...
- Tools are registered in `mcp/src/index.ts` (58 tools + 5 resources today).
